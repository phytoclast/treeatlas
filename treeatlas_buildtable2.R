...
M <- MAP/(Deficit + MAP - Surplus +0.0001)
vTclx <- velox(Tclx)
vTgs <- velox(Tgs)
vTc <- velox(Tc)
vTcl <- velox(Tcl)
...
line <- cbind(file = 'name',taxon = 'name',name = 'name', Tclx_max=0, Tclx_min=0, Tgs_max=0, Tgs_min=0, Cindex_max = 0, Cindex_min=0,Tcl_max=0, Tcl_min=0, M_max=0, M_min=0, MAP_max=0, MAP_min=0)
...
e <- vMAP$extract(spp2, df=T, small = T)
e <- e[!is.na(e[,2]),]
#densityplot(e[,2], plot.points=FALSE, bw=2, lwd=2, col='RoyalBlue', xlab='M', ylab='Density', main='')
MAP_max <- quantile(e[,2], top)
MAP_min <- quantile(e[,2], bot)
...
line2 <- cbind(file = as.character(treelist[n,2]), taxon = as.character(treelist[n,3]),
               ...
               Tcl_max, Tcl_min, M_max, M_min, MAP_max, MAP_min)
line <- rbind(line, line2)

}
write.csv(line, 'output/treeclim.csv', row.names = FALSE)
#Tc----
